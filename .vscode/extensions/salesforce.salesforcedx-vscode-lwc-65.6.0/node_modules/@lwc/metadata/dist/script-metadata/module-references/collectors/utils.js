...
function getModuleReference(moduleSpecifier, sourceNode, state, referenceType) {
    const { fileName, namespaceMapping, npmModuleMapping, bundleType } = state;
    ...
    const npmModuleNamespacedSpecifier = !utils.isEmptyObject(npmModuleMapping)
        ? npmModuleMapping[moduleSpecifier]
        : undefined;
    if (!utils.isUndefined(npmModuleNamespacedSpecifier)) {
        if (bundleType === 'platform') {
            throw new errors.MetadataCollectionError([
                createReferenceDiagnostic(sourceNode, fileName, errors.Errors.INVALID_NPM_MODULE_MAPPING_BUNDLE_TYPE, []),
            ]);
        }
        ref.namespacedName = npmModuleNamespacedSpecifier;
    }
    return ref;
}
...
function createSalesforceModuleCollector(config) {
    ...
    return {
        ...
        collect(value, node, state) {
            ...
            if (!config.disableDefaultImportAssert) {
